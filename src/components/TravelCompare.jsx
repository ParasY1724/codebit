...
const TravelComparison = () => {
  ...
  const [activeStyle, setActiveStyle] = useState('Solo Traveler'); 
  ...
  const getFilteredContent = (tab) => {
    if (!activeStyle) {
      return (
        <div className="p-4 bg-gray-100 rounded-md text-gray-600">
          Please select a travel style to view details for '{tab.name}'.
        </div>
      );
    }
            ...
            {travelStyles.map(style => (
              <div
                ...
                className={`border rounded-lg overflow-hidden ${
                  activeStyle === style.name ? 'shadow-2xl' : ''
                } ${activeStyle && activeStyle !== style.name ? 'opacity-80' : ''}`}
              >
  ...
  return (
    <div className="bg-white font-sans max-w-6xl mx-auto p-6">
      ...
      <div className="flex flex-wrap mb-4 gap-2">
        {travelStyles.map(style => (
          <button
            ...
            className={`px-6 py-2 font-medium rounded-full transition-all flex items-center ${
      ...
      <div className="transition-all duration-300 ease-in-out">
        {getFilteredContent(contentTabs.find(tab => tab.name === activeTab) || contentTabs[0])}
      </div>
    </div>
  );
};
